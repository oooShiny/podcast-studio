...
function handlePutStudioSettings(req, res) {
  ...
  req.on("end", () => {
    try {
      const { role, brandName, accentColor, bgColor, surfaceColor, textColor, textDimColor, menuTextColor, logoUrl, backgroundUrl, disabledTabs, guestPrepAccess, guestChatTab, memberChatTab } = JSON.parse(body);
      if (role !== "host") {
...
module.exports = {
  routes: [
    ...
    { method: "PUT", match: (url) => url === "/api/studio-settings", handler: handlePutStudioSettings },
    { method: "POST", match: (url) => url === "/api/studio-settings/upload", handler: handleUploadBrandingImage },
