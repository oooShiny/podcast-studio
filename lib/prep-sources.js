...
function handleUploadSource(req, res) {
  if (process.env.DEMO_MODE) {
    ...
    res.end(JSON.stringify({ error: "File uploads are disabled in this demo — add a URL/bookmark source instead" }));
    return;
  }
...
function handleDeleteSource(req, res, role) {
  // DELETE /api/prep-sources/{game}/{id}?user=
  const parts = req.url.replace(/\?.*$/, "").split("/");
...
function handleRenameSource(req, res, role) {
  // PATCH /api/prep-sources/{game}/{id}  body: { name, user }
  const parts = req.url.replace(/\?.*$/, "").split("/");
...
function handleTagSeasonSource(req, res) {
  ...
  req.on("end", () => {
    try {
      const { id, game, season } = JSON.parse(body);
...
module.exports = {
  routes: [
    ...
    { method: "POST", match: (url) => url === "/api/prep-sources/upload", handler: requireAuth(handleUploadSource) },
    { method: "POST", match: (url) => url === "/api/prep-sources/url", handler: requireAuth(handleAddUrlSource) },
    { method: "POST", match: (url) => url === "/api/prep-sources/tag-season", handler: requireHost(handleTagSeasonSource) },
    { method: "DELETE", match: (url) => url.startsWith("/api/prep-sources/"), handler: requireAuth(handleDeleteSource) },
    { method: "PATCH", match: (url) => url.startsWith("/api/prep-sources/"), handler: requireAuth(handleRenameSource) },
  ],
  init,
};
